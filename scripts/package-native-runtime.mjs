...
const targets = {
  "macos-arm64": {
    buildDir: "build/native-macos-arm64-appleclang-release-metal",
    platform: "macos",
    family: "macos",
    minimum: "13.0",
    host: "libeffindom_v2_macos_native_host.a",
    common: "libeffindom_v2_native_common.a",
    core: "libeffindom_core.dylib",
    ui: "libeffindom_ui.dylib",
    sdl: "libSDL3.0.dylib",
    system: [
      "Metal.framework",
  ...
  "macos-x64": {
    buildDir: "build/native-macos-x64-appleclang-release-metal",
    platform: "macos",
    family: "macos",
    minimum: "13.0",
    host: "libeffindom_v2_macos_native_host.a",
  ...
  "windows-arm64": {
    buildDir: "build/native-windows-arm64-msvc-release-d3d",
    platform: "windows",
    family: "windows",
    minimum: "10.0.17763",
    host: "effindom_v2_windows_native_host.lib",
    common: "effindom_v2_native_common.lib",
    core: "effindom_core.dll",
    coreImport: "effindom_core.lib",
    ui: "effindom_ui.dll",
    uiImport: "effindom_ui.lib",
    sdl: "SDL3.dll",
    sdlImport: "SDL3.lib",
    system: [
      "shell32",
  ...
  "windows-x64": {
    buildDir: "build/native-windows-x64-msvc-release-d3d",
    platform: "windows",
    family: "windows",
    minimum: "10.0.17763",
    host: "effindom_v2_windows_native_host.lib",
...
function findUnique(roots, name) {
  ...
  const matches = roots
    ...
    .filter(
      (path) =>
        basename(path) === name &&
        !excluded.some((part) => path.includes(part)),
    );
  if (matches.length !== 1)
    fail(
      `expected one ${name}, found ${matches.length}: ${matches.join(", ")}`,
    );
  return matches[0];
}
function input(source, path, role, executable = false) {
  return { source, path, role, executable };
}
...
if (
  ...
  fail(
    `usage: package-native-runtime.mjs --target <${Object.keys(targets).join("|")}> --source-commit <sha> --destination <directory>`,
  );
}
...
const files = [
  input(
    from("platformHost", target.host),
    `sdk/lib/${target.host}`,
    "host-library",
  ),
  input(
    from("commonHost", target.common),
    `sdk/lib/${target.common}`,
    "host-library",
  ),
  input(
    from("core", target.core),
    `runtime/lib/${target.core}`,
    "runtime-library",
  ),
  input(from("ui", target.ui), `runtime/lib/${target.ui}`, "runtime-library"),
  input(fromDeps(target.sdl), `runtime/lib/${target.sdl}`, "runtime-library"),
  input(
    join(root, "v2/native/common/src/NativeApplicationMain.cpp"),
...
for (const [name, output, buildOutput] of [
  ...
  [target.sdlImport, "sdk/lib", "dependency"],
]) {
  if (name)
    files.push(
      input(
        buildOutput === "dependency" ? fromDeps(name) : from(buildOutput, name),
        `${output}/${name}`,
        "host-library",
      ),
    );
...
for (const header of walk(join(root, "v2/native/common/include"))) {
  files.push(
    input(
      header,
      `sdk/include/${relative(join(root, "v2/native/common/include"), header).split(sep).join("/")}`,
      "launcher",
    ),
  );
}
...
for (const library of dependencyLinkInputs) {
  files.push(
    input(
      library,
      `sdk/deps/${relative(dependencyRoot, library).split(sep).join("/")}`,
      "host-library",
    ),
  );
}
for (const font of walk(join(root, "v2/fonts"))) {
  files.push(
    input(
      font,
      `runtime/assets/fonts/${relative(join(root, "v2/fonts"), font).split(sep).join("/")}`,
      "runtime-asset",
    ),
  );
}
...
const dependencyPath = (stem) => {
  ...
  const library = dependencyLinkInputs.find((path) =>
    names.includes(basename(path).toLowerCase()),
  );
  if (!library)
    fail(`required native link input is missing: ${names.join(" or ")}`);
