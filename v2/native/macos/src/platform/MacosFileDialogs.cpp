...
struct MacosFileDialogs::Request {
...
struct MacosFileDialogs::SharedState {
...
MacosFileDialogs::MacosFileDialogs(
    SDL_Window* window,
...
bool MacosFileDialogs::HandleEvent(const SDL_Event& event) {
...
void MacosFileDialogs::CompleteForTesting(NativeFileDialogCompletion completion) {
    std::uint64_t generation = 0U;
    {
        std::lock_guard lock(state_->mutex);
...
void MacosFileDialogs::DialogCompleted(void* userdata, const char* const* file_list, int filter) {
...
void MacosFileDialogs::QueueCompletion(
