...
MacosDropTarget::MacosDropTarget(SDL_Window* window, Engine& engine) : window_(window), engine_(engine) {}

bool MacosDropTarget::HandleEvent(const SDL_Event& event) {
...
void MacosDropTarget::Clear() {
    active_ = false;
    entered_ = false;
    items_.clear();
}

void MacosDropTarget::Begin(float x, float y) {
    Clear();
    active_ = true;
    x_ = x;
    y_ = y;
...
void MacosDropTarget::AddFile(const char* path_value) {
...
void MacosDropTarget::AddText(const char* text_value) {
...
std::uint32_t MacosDropTarget::Dispatch(std::uint32_t event_type) const {
...
std::vector<std::uint8_t> MacosDropTarget::EncodePayload() const {
